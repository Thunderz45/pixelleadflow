  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fade-in print:p-0 print:bg-white">
      <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] border border-outline-variant shadow-2xl flex flex-col overflow-hidden relative print:max-w-none print:max-h-none print:shadow-none print:border-none print:rounded-none">
        ...
        {/* Puppeteer Headless Scan Banner */}
        <div className="px-6 py-2.5 bg-slate-900 text-slate-200 border-b border-slate-800 flex items-center justify-between text-[11px] font-medium print:hidden">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-white">Puppeteer Headless Scan:</span>
            <span className="text-emerald-300 font-semibold">{report.webScan?.engine || "Puppeteer Engine Active"} • {report.scanStatus || "DOM Scanned Live"}</span>
          </div>
          {report.scannedUrl && report.scannedUrl !== "No Website Listed" && (
            <span className="font-mono text-[10px] bg-slate-800 border border-slate-700 px-2 py-0.5 rounded text-emerald-300">
              Target: {report.scannedUrl}
            </span>
          )}
        </div>
